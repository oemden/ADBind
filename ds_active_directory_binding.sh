#!/bin/sh
## Script borowed from DeployStudio modified by oem
## WARNING - psw in base64 encoded.

histchars=

SCRIPT_NAME=`basename "${0}"`

echo "${SCRIPT_NAME} - v2.1 ("`date`")"
...
AD_DOMAIN="hq.example.com"
COMPUTER_ID="$(/usr/sbin/scutil --get ComputerName)" # WARNING Device Needs to be named BEFORE
COMPUTERS_OU="OU=MAC,OU=Computers,OU=HQ,DC=hq,DC=example,DC=com"
ADMIN_LOGIN="administrateur"
ADMIN_PWD="$(echo UGFzc3cwcmQK | base64 --decode )" ## Yes not that safe but better than in plain text.
MOBILE="enable"
...
then
  ...
  sleep 1
  dsconfigad -passhqerval ${PASSWORD_hqERVAL} 2>&1
  if [ -n "${ADMIN_GROUPS}" ]
  then
    sleep 1
  ...
  then
    ...
    then
      DEFAULT_REALM=`more /Library/Preferences/edu.mit.Kerberos | grep default_realm | awk '{ prhq $3 }'`
      if [ -n "${DEFAULT_REALM}" ]
      then
